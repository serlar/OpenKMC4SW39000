...
void Domain::procs2domain_3d()
{
  ...
  int num_x = xprd / this->lattice->xlattice;
  int num_y = yprd / this->lattice->ylattice;
  int num_z = zprd / this->lattice->zlattice;

  #define GET_LOW(id, p, n) ((id)*(n)/(p))

  int x_low = GET_LOW(myloc[0], procgrid[0], num_x);
  int y_low = GET_LOW(myloc[1], procgrid[1], num_y);
  int z_low = GET_LOW(myloc[2], procgrid[2], num_z);

  int x_hig = GET_LOW(myloc[0]+1, procgrid[0], num_x);
  int y_hig = GET_LOW(myloc[1]+1, procgrid[1], num_y);
  int z_hig = GET_LOW(myloc[2]+1, procgrid[2], num_z);

  subxlo = boxxlo + this->lattice->xlattice * x_low;
  subxhi = boxxlo + this->lattice->xlattice * x_hig;

  subylo = boxylo + this->lattice->ylattice * y_low;
  subyhi = boxylo + this->lattice->ylattice * y_hig;

  subzlo = boxzlo + this->lattice->zlattice * z_low;
  subzhi = boxzlo + this->lattice->zlattice * z_hig;
