...
Finish::Finish(SPPARKS *spk, int flag) : Pointers(spk)
{
  ...
  double time_other = timer->array[TIME_LOOP] -
                      (timer->array[TIME_COMM] + timer->array[TIME_OUTPUT] +
                       timer->array[TIME_APP] + timer->array[TIME_BORDER] +
                       timer->array[TIME_PROPENSITY] + timer->array[TIME_WAIT] +
                       timer->array[TIME_BARRIER] + timer->array[TIME_SOLVE] +
                       timer->array[TIME_UPDATE]);
  ...
  time = timer->array[TIME_BORDER];
  MPI_Allreduce(&time, &tmp, 1, MPI_DOUBLE, MPI_SUM, world);
  time = tmp / nprocs;
  if (me == 0)
  {
    if (screen)
      fprintf(screen, "Border time (%%) = %g (%g)\n", time, time / time_loop * 100.0);
    if (logfile)
      fprintf(logfile, "Border time (%%) = %g (%g)\n", time, time / time_loop * 100.0);
  }

  time = timer->array[TIME_PROPENSITY];
  MPI_Allreduce(&time, &tmp, 1, MPI_DOUBLE, MPI_SUM, world);
  time = tmp / nprocs;
  if (me == 0)
  {
    if (screen)
      fprintf(screen, "Propensity time (%%) = %g (%g)\n", time, time / time_loop * 100.0);
    if (logfile)
      fprintf(logfile, "Propensity time (%%) = %g (%g)\n", time, time / time_loop * 100.0);
  }
  ...
  time = timer->array[TIME_WAIT];
  MPI_Allreduce(&time, &tmp, 1, MPI_DOUBLE, MPI_SUM, world);
  time = tmp / nprocs;
  if (me == 0)
  {
    if (screen)
      fprintf(screen, "Wait  time (%%) = %g (%g)\n", time, time / time_loop * 100.0);
    if (logfile)
      fprintf(logfile, "Wait  time (%%) = %g (%g)\n", time, time / time_loop * 100.0);
  }

  time = timer->array[TIME_BARRIER];
  MPI_Allreduce(&time, &tmp, 1, MPI_DOUBLE, MPI_SUM, world);
  time = tmp / nprocs;
  if (me == 0)
  {
    if (screen)
      fprintf(screen, "Barrier  time (%%) = %g (%g)\n", time, time / time_loop * 100.0);
    if (logfile)
      fprintf(logfile, "Barrier  time (%%) = %g (%g)\n", time, time / time_loop * 100.0);
  }
